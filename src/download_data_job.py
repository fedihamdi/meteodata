...
def download_cams_data_pol(output_filename, start_date, end_date):
    retriever = CAMSDataRetriever(start_date=start_date, end_date=end_date, type='forecast',
                                  variables=['alder_pollen', 'birch_pollen', 'grass_pollen',
...
def main():
    ...
    era5_process_th = multiprocessing.Process(target=download_era5_data_th, args=(era5_filename_th, start_date, end_date))
    era5_process_precip = multiprocessing.Process(target=download_era5_data_p, args=(era5_filename_p, start_date, end_date))

    cams_process_aq = multiprocessing.Process(target=download_cams_data_aq, args=(cams_filename_aq, start_date, end_date))
    cams_process_pol = multiprocessing.Process(target=download_cams_data_pol, args=(cams_filename_pol, start_date, end_date))
